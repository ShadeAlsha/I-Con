...
class KNNKernel(Kernel):
    ...
    def forward(self, features: torch.Tensor, labels: Optional[torch.Tensor] = None,
        ...
        if self.config.normalize:
            ...
            adj_matrix = adj_matrix / (deg + self.config.eps)
...
class LabelKernel(Kernel):
    ...
    def __init__(self, sparse: bool = True, config: Optional[KernelConfig] = None):
        super().__init__(config)
        self.sparse = sparse
    ...
    def forward(self, features: Optional[torch.Tensor] = None,
        ...
        labels = labels.to(device=self.config.device)
        n = labels.size(0)
        ...
        if self.config.normalize:
            ...
            adj_matrix = adj_matrix / (row_sums + self.config.eps)
            if self.sparse and not self.config.symmetric:
                adj_matrix = adj_matrix.to_sparse()
...
class AugmentationKernel(Kernel):
    ...
    def forward(
        ...
        kernel_matrix = torch.zeros(batch_size, batch_size, 
                                  device=device, dtype=self.config.dtype)
        
        # If block is provided, create block diagonal matrix
        if self.block is not None:
            block = self.block.to(device=device, dtype=self.config.dtype)
